...
type FieldAction =
    | Map of Expr     // Map over the elements,           input function is 'a -> 'a for a' []
    | Filter of Expr  // Filter out elements,             input function is 'a -> bool for a' []
    | Collect of Expr // Collect up elements              input function is 'a -> 'a [] for a' []
    | Default of Expr // Default (if none or empty),      input function is unit -> 'a [] for a '[]
    | Function of Expr   // Transform at the value level, input function is 'a [] -> 'a [] for a '[] (same as map for single values)
    | Add of Expr     // Adds to the current collection,  input function is unit -> 'a [] for a '[]
with 
...
// example: let m = (getMethod <@ failwith message @>).MakeGenericMethod([|utype|])
let getMethod = 
    function
...
let genrateRecordDeepCopyFunction<'T> rcs (funcs: FieldUpdaters) : ('T -> 'T) = 
    let filledMappedFuncs = fillFieldUpdaterParents funcs
    let expr = makeSingleArgCopyFunExpr rcs typeof<'T> filledMappedFuncs []
    let castExpr : Expr<'T -> 'T> = expr |> Expr.Cast
    let compiledExpr = (castExpr |> toLinq).Compile()
    fun (v : 'T) -> compiledExpr.Invoke(v)

let genrateRecordDeepCopyFunctionWithArgs<'U,'T> rcs (replacmentVarName: string) (funcs: FieldUpdaters) : ('U -> 'T -> 'T) = 
    ...
    let contents = makeSingleArgCopyFunExpr rcs typeof<'T> filledMappedFuncs []
